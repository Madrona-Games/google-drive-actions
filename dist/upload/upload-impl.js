...
async function uploadImpl() {
    ...
    const inputDestinationFolderID = utils.getInputAsString(constants_1.Inputs.DestinationFolderID);
    const inputDestinationFolderPath = utils.getInputAsString(constants_1.Inputs.DestinationFolderPath);
    const inputFilePath = utils.getInputAsString(constants_1.Inputs.FilePath, {
        required: true,
    });
    ...
    if (!folderID) {
        folderID = await (0, find_impl_1.findFileID)(inputDestinationFolderPath, inputGoogleDriveRootFolderId, googleDriveService);
    }
    const id = await googleDriveService.uploadFile(folderID, inputFilePath);
    core.setOutput(constants_1.Outputs.FileID, id);
}
exports.uploadImpl = uploadImpl;
//# sourceMappingURL=upload-impl.js.map